...
// Delete a hiring form submission by ID
router.delete("/submissions/:id", async (req, res) => {
  try {
    const submissionId = req.params.id;

    // Find the submission by ID
    const submission = await HiringForm.findById(submissionId);
    ...
    await HiringForm.findByIdAndDelete(submissionId);

    res.json({ message: "Submission deleted successfully" });
  } catch (error) {
    res.status(500).json({
      message: "Error deleting hiring form submission",
      error: error.message,
    });
  }
});
